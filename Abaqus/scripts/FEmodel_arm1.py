...
def simulation(u,name = 'temp',**kwargs):
    ...
    x11s = L/2 + (R+2*r_s)*np.cos(theta00)
    y11s = (R+2*r_s)*np.sin(theta00)
    ############################################################## Parts - Circle ###################################################
    part_circle(R,L)
    ...
    if sym:
        ...
        xright = list(np.linspace(0,x11s,250))
    ...
    x_central = L/2+ (R+r_s)*np.cos(theta00)
    y_central = (R+r_s)*np.sin(theta00)
    s1.ArcByCenterEnds(center=(x_central, y_central), point1=(x11s, y11s), point2=(x00, y00), direction=CLOCKWISE)
    ...
    edge5 = p.edges.findAt(((x11s,y11s,0.0),))
    
    p.Set(edges=edge1+edge2+edge3+edge4+edge5, name='ligament')
    ...
    edge5 = a.instances['ligament-1'].edges.findAt(((x11s,y11s,0.0),))
    

    a.Set(edges=edge1+edge2+edge3+edge4+edge5, name='ligament')

    # surfaces for contact
    a.Surface(side2Edges=edge1+edge4, name='ligament_left')
    a.Surface(side2Edges=edge3+edge5, name='ligament_right')
