...
config = configAll.arm_configs['arm1']
...
class one_ligament_asym():
    ...
    def __init__(self, R=5, L=20,theta0 = -60, theta1=-60, theta00 = 130, theta11=-70, r_s = 0.5, n = 10,ds = mix_space(['poly']*10)):
        ...
        self.r_s = r_s 
        self.x1 = -L/2+ (R+2*r_s)*np.cos(self.theta0)
        self.y1 = (R+2*r_s)*np.sin(self.theta0)
    ...
    def plot_geo(self,a, points = None,filename=None,ax=None):
        ...
        #plt.plot(x_ligleft, y_ligleft, c = color_lig,linewidth=3)
        #plt.plot(x_ligright, y_ligright, c = color_lig,linewidth=3)
        x_lig = [np.flip(x_ligleft),x_ligright]
        y_lig = [np.flip(y_ligleft),y_ligright]

        #ligament on the small circle
        theta = np.linspace(theta0,theta0 - np.radians(180),25)
        x_central = -L/2+ (R+self.r_s)*np.cos(self.theta0)
        y_central = (R+self.r_s)*np.sin(self.theta0)
        x_circle1 = x_central + self.r_s*np.cos(theta)
        y_circle1 = y_central + self.r_s*np.sin(theta)
        #plt.plot(x_circle1, y_circle1, c= color_lig,linewidth=3)
        x_lig.insert(1,np.flip(x_circle1))
        y_lig.insert(1,np.flip(y_circle1))
        ...
        #plt.plot(x, y, c = color_lig,linewidth=3)
        x_lig.insert(2,x)
        y_lig.insert(2,y)
        ...
        ax.scatter(x_ligleft[-1], y_ligleft[-1], c = color_lig,s=40,zorder=10)
        ax.scatter(x_ligright[-1], y_ligright[-1], c = color_lig,s=40,zorder=10)
...
def asym_poly_designs(n_designs, n=8,ds_name=['poly']*10,points_fix=None):
    ligs = []
    circles = []
    points = []
    while len(ligs)<n_designs:
        theta0 = random.uniform(-90,0)
        theta1 = random.uniform(20,90)
        theta00 = random.uniform(90,180)
        theta11 = random.uniform(20,90)
        R = random.uniform(3,7)
        r_s = random.uniform(0.1,R/10)

        chiral = one_ligament_asym(R=R, L=20,theta0 = theta0, theta1=theta1, theta00 = theta00, theta11=theta11, r_s=r_s, n = n, ds = mix_space(ds_name))
...
def show_geo_abaqus(path, index,filename=None,ax=None):
    ...
    show_geo(designs_circle,designs_poly,designs_points,ds = config.ds,filename=filename,ax=ax)


if __name__=='__main__':
    ds = ['poly']*10
    ligs, circles, points = asym_poly_designs(n_designs = 2, n=8, ds_name=ds)
    for i in range(2):
