...
def assembly(L):
    ...
    a = mdb.models['Model-1'].rootAssembly
    a.rotate(instanceList=('circle-1', ), axisPoint=(-L/2, 0.0, 0.0), axisDirection=(0.0, 0.0, 1.0), angle=180.0)
...
def simulation(u,name = 'temp',**kwargs):
    ...
    a = kwargs['a']
    ds = kwargs['ds']
    R, L,theta0,theta1 = kwargs['R'] ,kwargs['L'], kwargs['theta0'], kwargs['theta1']  
    ...
    #xmid,ymid = -L/2+R*math.cos(start/2+end/2), R*math.sin(start/2+end/2)
    ...
    xleft = list(np.linspace(x0,0,250)) 
    ...
    p.Set(edges=edge1+edge2+edge3, name='ligament')
    ...
    a.Set(edges=edge1+edge2+edge3, name='ligament')

    # surfaces for contact
    a.Surface(side2Edges=edge1+edge2, name='ligament_left')
    a.Surface(side2Edges=edge3+edge2, name='ligament_right')
